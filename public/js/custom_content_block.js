...
function restoreParms() {
    ...
    if(messageType) {
      if(messageType === 'text-area'){
        ...
        document.getElementById('formActionText').value = objData.u.formActionText;
        document.getElementById('formActionLink').value = objData.u.formActionLink;
...
function paint(messageType) {
  ...
  if(messageType === 'text-area'){
    ...
    const actionText = document.querySelector('#formActionText').value.trim();
    const actionLink = document.querySelector('#formActionLink').value;

    const actionButton = actionText
      ? `<a href="${actionLink}"
            style="text-align:center;margin-top:10px;width:250px;display:inline-block;background:#7c4dff;color:#fff;padding:10px 24px;border-radius:999px;font-size:14px;font-weight:600;text-decoration:none;">
            ${actionText}
        </a>`
      : '';
    ...
    let content = `
      <div style="width:300px; margin-left:auto; margin-right:auto">
        <div style="padding:5px; background-color:#FFF">
          ${document.querySelector('#txtBlock2').value}
        </div>
        ${img}
        ${actionButton}
      </div>
    ...
    sdk.setData({
      'type': messageType,
      'typeName': 'Text with Photo/CTA',
      'u' : {
        'txtBlock2' : document.querySelector('#txtBlock2').value,
        'formActionLink' : document.querySelector('#formActionLink').value,
        'formActionText' : document.querySelector('#formActionText').value,
